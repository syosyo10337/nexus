...
export const useBreadcrumbStore = create<BreadcrumbState>(set => ({
  ...
  setItems: (next) => {
    if (next.length > 4) {
      console.warn('[Breadcrumb] supports up to 4 levels. Extra levels are ignored.')
...
import { convertBase64ToFiles, convertFilesToBase64 } from '@/shared/utils/adapters/base64'
...
export type EventCreateWizardStepKey = typeof EVENT_WIZARD_STEPS[number]['key']
```
