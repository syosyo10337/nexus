...
class InstagramRenderer implements Renderer {
  ...
  private renderPerformers(
    performers: Performer[],
    block: PerformerListBlock
  ): string {
    return performers
      .map((p) => {
        let line = p.name;
    ...
    if (block.formatting) {
      ...
      for (const fmt of sorted) {
        if (fmt.type === "mention" && fmt.value) {
          text =
            text.slice(0, fmt.start) + `@${fmt.value}` + text.slice(fmt.end);
        }
      }
    }
...
class WebsiteRenderer implements Renderer {
  ...
  private renderBlock(block: ContentBlock, event: Event): string {
    switch (block.type) {
      ...
      case "list":
        const tag =
          block.style === "numbered" || block.style === "numbered_dot"
            ? "ol"
            : "ul";
...
const templates: Record<TemplateType, ContentBlock[]> = {
  soy_poy_standard: [
    ...
    {
      type: "paragraph",
      text: "音楽、朗読、コント、ダンス、ポエトリー、スピーチなど。...",
    },
