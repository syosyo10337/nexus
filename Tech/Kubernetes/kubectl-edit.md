...
vimエディターが起動されて、インタラクティブに編集できます。

- 🔧 開発・デバッグ時の素早い変更
- 📝 複数のフィールドを一度に変更したい
- 👀 リソース全体の設定を確認しながら編集したい
- 🚀 アドホックな変更（一時的な調整）
のような時kubectl editを使うことができます。

```bash
kubectl edit <resource_name>
```

## コマンドラインで編集する(`kubectl patch`)

- 🤖 自動化・CI/CDパイプライン内での変更
- 📜 スクリプト化された運用
- 🎯 特定フィールドのみを正確に変更
- 🔁 複数リソースへの同じ変更を一括適用
- ✅ バリデーションが必要な変更

のような時kubectl patchを使うことができます。

```bash
kubectl patch <resource_name>  -p '{"spec":{"replicas":2}}'
```
