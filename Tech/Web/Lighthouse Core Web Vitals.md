...
| **MP4 (H.265/HEVC)** | 1MB以下    | 3MB        | ⚠️ Safari は対応、Chrome/Edge はハードウェア依存で部分対応。Web 用途では非標準 |
...
これらだけで転送量は25MBから**5〜8MB程度**まで削減できる見込みです。CDNを入れるのはその後でも十分効果が出ます。
...
`srcset` の `400w` は「このファイルの画像幅は 400px」という**ファイルの仕様の宣言**。CSS ピクセルでも DPR でもない。

`sizes` とは何か — `sizes` は CSS ではなく、**ブラウザへの事前ヒント**。「このページではこの画像が何 px 幅で表示されますか？」という情報を伝える。

なぜ必要か — ブラウザは HTML をパースした**直後**（CSS がまだ読み込まれていない段階）に、どの画像ファイルをフェッチするか決める。このとき CSS レイアウトがわからないため、`sizes` で開発者が教えてあげる必要がある。

```html
sizes="
  (max-width: 600px) 100vw,   ← ビューポートが ≤600px のとき → 表示幅 = ビューポート幅 100%
  (max-width: 1200px) 50vw,   ← ビューポートが ≤1200px のとき → 表示幅 = ビューポート幅の 50%
...
ビューポート = 1000px, DPR = 2x の場合:
  1. sizes を上から評価 → (max-width: 1200px) に該当 → 表示幅 = 50vw = 500px
  2. 必要な解像度 = 500px × DPR 2 = 1000px
  3. srcset から 1000w 以上で最も小さい → 1200w の画像を選択
```

よくあるミス:

- `sizes` を書かないと `100vw`（ビューポート全幅）がデフォルトになる → モバイルではほぼ正しいが、デスクトップで狭いカラムに置いた画像を巨大なファイルでフェッチしてしまう
- CSS のメディアクエリと別物なので、CSS の `max-width: 600px` とは連動しない。両方を個別に管理する必要がある
